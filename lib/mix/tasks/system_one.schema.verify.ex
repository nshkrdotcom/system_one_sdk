defmodule Mix.Tasks.Typesafe.Schema.Verify do
  ...
  def run(args) do
    ...
    if remaining != [] or invalid != [],
      do: Mix.raise("usage: mix typesafe.schema.verify [--output directory]")

    directory = Keyword.get(opts, :output, "priv/json_schema")

    case SystemOneSDK.Schema.verify(directory) do
      :ok ->
        Mix.shell().info("TypeSafe wire schemas are current")

      {:error, files} ->
        Mix.raise(
          "Stale or unexpected schemas: #{Enum.join(files, ", ")}; run mix typesafe.schema.export"
        )
    end
  end
end
