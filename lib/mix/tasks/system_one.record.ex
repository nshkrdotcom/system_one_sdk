defmodule Mix.Tasks.Typesafe.Record do
  ...
  def run(args) do
    ...
    client = SystemOneSDK.new_client(client_opts)
    ...
    response =
      SystemOneSDK.evaluate!(
        ...
        billing: SystemOneSDK.noul("Is this about billing?"),
        team:
          SystemOneSDK.choice("Primary department?",
            billing: "Invoices",
            technical: "Product failures",
            sales: "New purchases"
          ),
        urgency: SystemOneSDK.score("Urgency?", ["Routine", "Urgent"])
      )
  ...
  defp write!(directory, filename, data) do
    bytes = data |> SystemOneSDK.JSON.ordered() |> Jason.encode!(pretty: true)
