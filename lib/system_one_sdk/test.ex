defmodule SystemOneSDK.Test do
  @moduledoc """
  ...
  child tasks. Call `verify!/1` before the owner exits, and `close/1` when done.
      ...
      result = SystemOneSDK.evaluate!(client, "synthetic example", urgent: SystemOneSDK.noul("Urgent?"))
  ...
  alias SystemOneSDK.Test.{ContractError, Scenario, Transport}
  ...
  defp isolate_runtime(client, scenario) do
    ...
    pristine_client = Pristine.Client.from_context(context)

    provider_client =
      case client.provider_client do
        %{context: _provider_context, pristine_client: _provider_runtime} = provider_client ->
          %{
            provider_client
            | context: context,
              pristine_client: pristine_client
          }
  ...
  defp scenario!(_), do: raise(ArgumentError, "expected a SystemOneSDK.Test client")
end
