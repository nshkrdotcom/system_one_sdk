defmodule SystemOneSDK.Schema do
  @moduledoc """
  ...
  """
  @roots %{
    "system-one-request.json" => "SystemOneRequest",
    "system-one-response.json" => "SystemOneResponse",
    "models-response.json" => "ModelMetadataList"
  }

  @spec source_path() :: String.t()
  def source_path, do: Application.app_dir(:system_one_sdk, "priv/upstream/openapi.json")
    ...
    Map.new(@roots, fn {filename, root} ->
      ...
      {filename,
       %{
         ...
         "$defs" => definitions,
         "title" => root,
         "x-system-one-sdk-version" => SystemOneSDK.version(),
  ...
  def export(directory, source \\ source_path()) do
    ...
    |> Enum.each(fn {filename, document} ->
      ...
      try do
        content = document |> SystemOneSDK.JSON.ordered() |> Jason.encode!(pretty: true)
  ...
  def verify(directory, source \\ source_path()) do
    ...
    stale =
      Enum.flat_map(expected, fn {filename, document} ->
  ...
  defp validate_references!(value, root) when is_map(value) do
    Enum.each(value, fn
      {key, reference} when key in ["$ref", "$dynamicRef"] ->
      ...
      Enum.reduce(tokens, root, fn token, value ->
        ...
        case value do
          value when is_map(value) ->
            Map.fetch!(value, token)

          value when is_list(value) ->
            resolve_index!(value, token)

          _ ->
            raise ArgumentError, "unresolved JSON pointer in schema reference"
        end
      end)
  ...
  defp resolve_index!(value, token) do
