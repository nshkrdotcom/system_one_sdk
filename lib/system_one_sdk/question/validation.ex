defmodule SystemOneSDK.Question.Validation do
  @moduledoc false
  alias SystemOneSDK.{Error, JSON}
  alias SystemOneSDK.Question.{Choice, Noul, Score}
  ...
  def compile(question, path) do
    with {:ok, type, instructions, criteria, extra} <- fields(question, path),
      ...
      wire =
        if Map.has_key?(definition, :wire_criteria),
          do: Map.put(wire, "criteria", definition.wire_criteria),
          else: wire
  ...
  defp fields(%SystemOneSDK.Noul{} = q, _), do: {:ok, "noul", q.instructions, q.criteria, %{}}
  defp fields(%SystemOneSDK.Choice{} = q, _), do: {:ok, "choice", q.instructions, q.criteria, %{}}
  defp fields(%SystemOneSDK.Score{} = q, _), do: {:ok, "score", q.instructions, q.criteria, %{}}
  ...
  defp raw_fields(map, path) do
    case map["type"] do
      ...
      _ ->
        invalid(path ++ ["type"], "must be a nonblank string")
    end
  end
  ...
  defp criteria("noul", value, path) do
    with {:ok, pairs} <- JSON.keyed_pairs(value, path),
      ...
      {:ok,
       %{criteria: pairs, wire_criteria: Map.new(pairs, fn {key, v} -> {JSON.wire_key(key), v} end)}}
    end
  end
    ...
    with {:ok, pairs} <- JSON.keyed_pairs(value, path),
      ...
      {:ok,
       %{
         criteria: pairs,
         option_keys: keys,
         option_order: Enum.map(pairs, &elem(&1, 0)),
  ...
  defp level(value, index, path) do
    with {:ok, normalized} <- JSON.description(value, path),
      ...
      description =
        if is_map(normalized), do: Map.get(normalized, "description", normalized), else: normalized
  ...
  defp count(values, min, max, path) do
    if length(values) in min..max,
      do: :ok,
      else:
        invalid(path, "requires #{min}..#{max} entries", %{
          count: length(values),
          min: min,
          max: max
        })
  ...
  defp invalid(path, reason, details \\ %{}),
    do: {:error, Error.invalid_request(path, reason, details)}
end
