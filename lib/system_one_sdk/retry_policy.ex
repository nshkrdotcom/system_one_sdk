defmodule SystemOneSDK.RetryPolicy do
  @moduledoc """
  ...
  @spec new(keyword() | map()) :: {:ok, t()} | {:error, SystemOneSDK.Error.t()}
  def new(opts \\ []) do
    ...
    case validate(policy) do
      :ok -> {:ok, policy}
      {:error, message} -> {:error, SystemOneSDK.Error.configuration(message)}
    end
  rescue
    KeyError -> {:error, SystemOneSDK.Error.configuration("unknown retry policy option")}
  end
  ...
  @spec merge(t() | false | nil, t() | keyword() | map() | false | nil) ::
          {:ok, t() | false} | {:error, SystemOneSDK.Error.t()}
  ...
  def merge(_base, _override),
    do: {:error, SystemOneSDK.Error.configuration("invalid retry override")}
  ...
  defp override_attrs(attrs) when is_list(attrs) do
    cond do
      not Keyword.keyword?(attrs) ->
        {:error, SystemOneSDK.Error.configuration("retry override must be a keyword list")}

      length(Keyword.keys(attrs)) != length(Enum.uniq(Keyword.keys(attrs))) ->
        {:error,
         SystemOneSDK.Error.configuration("duplicate retry override options are not allowed")}
