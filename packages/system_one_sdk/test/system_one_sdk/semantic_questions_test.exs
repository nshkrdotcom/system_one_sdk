defmodule SystemOneSDK.SemanticQuestionsTest do
  use ExUnit.Case, async: true

  alias SystemOneSDK.{Error, JSON, Prepared, Question}
  ...
  test "strict constructors have tuple and raising forms without changing legacy constructors" do
    ...
    assert %Noul{} = SystemOneSDK.noul("Urgent?")
    assert %Choice{} = SystemOneSDK.choice("Team?", billing: nil, technical: nil)
    assert %Score{} = SystemOneSDK.score("Severity?", ["Low", "High"])
    assert %SystemOneSDK.Choice{} = SystemOneSDK.Choice.new(%{"only" => nil})
    assert %SystemOneSDK.Score{} = SystemOneSDK.Score.new(["only"])
  ...
  test "choice pairs keep order including large maps and atoms restore only through finite keys" do
    ...
    assert {:ok, %Prepared{} = prepared} = SystemOneSDK.prepare(team: question)
  ...
  test "normalized collisions are rejected before conversion to maps" do
    ...
    q = SystemOneSDK.noul("Question?")

    assert {:error, %Error{path: ["questions", "q"]}} =
             SystemOneSDK.prepare([{:q, q}, {"q", q}])
  ...
  test "structured levels preserve labels and descriptions and extras cannot override fields" do
    ...
    assert {:ok, prepared} = SystemOneSDK.prepare(severity: q)
  ...
  test "raw future questions remain forward compatible and Prepared is reusable" do
    ...
    assert {:ok, ^p} = SystemOneSDK.prepare(p)
    assert p.definitions["future"].type == "future"
    assert {:error, _} = SystemOneSDK.prepare(%{})
    assert {:error, _} = SystemOneSDK.prepare(%{" " => SystemOneSDK.noul("Yes?")})
  end
  ...
  test "improper JSON arrays, pair lists and level lists return validation tuples" do
    ...
    assert {:error, %Error{type: :invalid_request}} =
             SystemOneSDK.prepare([{:q, SystemOneSDK.noul("Q?")} | :invalid])
