defmodule SystemOneSDK.SemanticResponseTest do
  use ExUnit.Case, async: true
  alias SystemOneSDK.{Answer, Error, Response, SemanticResponse, SystemOneResponse}

  defp questions do
    SystemOneSDK.prepare!(
      urgent: SystemOneSDK.noul("Urgent?"),
      team: SystemOneSDK.choice("Team?", billing: "Payments", support: "Product"),
      level: SystemOneSDK.score("Severity?", [{"Low", "Cosmetic"}, {"High", "Blocking"}])
    )
  end
  ...
  test "Choice ranking uses explicit caller tie order and margin is not confidence" do
    answer = %SystemOneSDK.ChoiceAnswer{
      choice: :z,
      confidence: 0.9,
  ...
  test "rounded expectation can be the least likely level" do
    answer = %SystemOneSDK.ScoreAnswer{
      score: 1.0,
      confidence: 0.4,
