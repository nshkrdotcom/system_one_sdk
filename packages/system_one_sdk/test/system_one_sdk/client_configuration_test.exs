defmodule SystemOneSDK.ClientConfigurationTest do
  use ExUnit.Case, async: false

  alias SystemOneSDK.Client

  setup do
    previous = Application.get_all_env(:system_one_sdk)

    on_exit(fn ->
      for {key, _value} <- Application.get_all_env(:system_one_sdk) do
        Application.delete_env(:system_one_sdk, key)
      end

      for {key, value} <- previous, do: Application.put_env(:system_one_sdk, key, value)
    end)
  ...
  test "explicit invalid endpoint settings fail instead of falling back" do
    for {base_url, message} <- [
      ...
      assert_raise SystemOneSDK.Error, message, fn ->
  ...
  test "explicit blank model fails instead of selecting a different provider model" do
    assert_raise SystemOneSDK.Error, ~r/model must not be blank/, fn ->
  ...
  test "application configuration is the runtime environment boundary" do
    Application.put_env(:system_one_sdk, :api_key, "configured")
    Application.put_env(:system_one_sdk, :base_url, "https://configured.test/")
    Application.put_env(:system_one_sdk, :default_model, "configured-model")
  ...
  test "explicit nil inherits application configuration like Python None" do
    ...
    assert %SystemOneSDK.RetryPolicy{} = client.retry
  end

  test "missing key raises a SystemOneSDK.Error" do
    Application.delete_env(:system_one_sdk, :api_key)
    assert_raise SystemOneSDK.Error, ~r/No API key/, fn -> Client.new() end
  end
end
