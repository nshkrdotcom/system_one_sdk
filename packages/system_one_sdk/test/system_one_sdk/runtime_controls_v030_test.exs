defmodule SystemOneSDK.RuntimeControlsV030Test do
  use ExUnit.Case, async: true

  alias SystemOneSDK.{Client, Error, RequestBudget, RetryPolicy, Test}
  ...
  test "request byte budget rejects before transport egress and counts UTF-8 serialized bytes" do
    ...
    assert {:error, %Error{type: :request_too_large} = error} =
             SystemOneSDK.evaluate(client, "é", questions())
  ...
  test "per-call request budget overrides client default" do
    ...
    assert {:ok, _} = SystemOneSDK.evaluate(client, "ok", questions(), max_request_bytes: 4096)
  ...
  test "semantic evaluation forwards the exact cancellation token to Pristine transport" do
    ...
    assert {:ok, _response} =
             SystemOneSDK.evaluate(client, "state", questions(), cancellation: token)
  ...
  test "partial per-call retry overrides inherit client max_retries at execution" do
    ...
    assert {:error, %Error{status: 500}} =
             SystemOneSDK.evaluate(client, "state", questions(), retry: [backoff_initial: 0.0])
  ...
  test "batch cancellation halts future scheduling" do
    ...
    consumer =
      Task.async(fn ->
        SystemOneSDK.evaluate_many(client, 0..20, questions(),
          cancellation: token,
          max_concurrency: 1,
          ordered: false
        )
