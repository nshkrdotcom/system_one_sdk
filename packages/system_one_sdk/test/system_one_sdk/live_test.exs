...
defmodule SystemOneSDK.LiveTest do
  use ExUnit.Case

  @moduletag :live

  alias SystemOneSDK.{Choice, Score}
  ...
  test "lists real models", %{client: client} do
    assert {:ok, response} = SystemOneSDK.list_models(client)
  ...
  test "runs real System One questions", %{client: client} do
    ...
    assert {:ok, response} = SystemOneSDK.system_one(client, state, questions)
