defmodule SystemOneSDK.PreparedV030Test do
  use ExUnit.Case, async: true

  alias SystemOneSDK.{Error, Prepared}
  alias SystemOneSDK.Question.Noul

  defp q(label), do: SystemOneSDK.noul(label)

  test "composition preserves deterministic execution order and caller keys" do
    prepared = SystemOneSDK.prepare!(alpha: q("A?"), beta: q("B?"))
  ...
  test "merge is right-biased without moving duplicate left positions" do
    left = SystemOneSDK.prepare!(a: q("left a"), b: q("left b"))
    right = SystemOneSDK.prepare!(b: q("right b"), c: q("right c"))
  ...
  test "fingerprint is versioned, stable, semantic, and recomputed by composition" do
    one = SystemOneSDK.prepare!(q: SystemOneSDK.choice("Pick", a: "A", b: "B"))
  ...
  test "composition re-runs semantic validation" do
    prepared = SystemOneSDK.prepare!(q: q("Q?"))

    assert {:error, %SystemOneSDK.Error{type: :invalid_request}} =
  ...
  test "map-valued extras fingerprint independently of map iteration order" do
    ...
    assert Prepared.fingerprint(SystemOneSDK.prepare!(q: q1)) ==
             Prepared.fingerprint(SystemOneSDK.prepare!(q: q2))
  end

  test "composition rejects invalid caller keys through the normal error surface" do
    prepared = Prepared.new!(a: SystemOneSDK.noul("A?"))

    assert {:error, %Error{type: :invalid_request}} =
             Prepared.put(prepared, 123, SystemOneSDK.noul("B?"))
