defmodule SystemOneSDK.OTP.ServerV040Test do
  ...
  defmodule Triage do
    ...
    defp classify(from, state, opts, inner) do
      questions = [route: SystemOneSDK.choice("Route?", billing: nil, support: nil)]
  ...
  defmodule Recursive do
    ...
    def handle_call(:run, from, state) do
      {:evaluate, {{from, 1}, "first", [continue: SystemOneSDK.noul("Continue?")]}, state}
    end

    @impl true
    def handle_evaluation({:ok, _response}, {from, 1}, state) do
      request = {{from, 2}, "second", [continue: SystemOneSDK.noul("Continue?")]}
  ...
  test "unsupported cancellation returns a typed error without crashing the server", %{
    ...
    client = SystemOneSDK.Client.new(api_key: "test-key", transport: UnsupportedTransport)
    pid = start_triage(client, supervisor, max_in_flight: 1)
