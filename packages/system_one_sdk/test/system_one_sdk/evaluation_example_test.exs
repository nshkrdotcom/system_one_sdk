...
defmodule SystemOneSDK.EvaluationExampleTest do
  use ExUnit.Case, async: true
  alias SystemOneSDK.Examples.Evaluation, as: Eval
  ...
  test "the runnable example evaluates through the real SDK and excludes state from reports" do
    client =
      SystemOneSDK.Test.client()
      |> SystemOneSDK.Test.stub(
    ...
    assert SystemOneSDK.Test.verify!(client) == :ok
  end
  ...
  test "unknown future answers become reported example failures, not a crashed report" do
    ...
    response = %SystemOneSDK.SystemOneResponse{
      model: "jev",
      usage: %SystemOneSDK.Usage{},
      answers: %{},
