defmodule SystemOneSDK.Telemetry do
  @moduledoc """
  Privacy-oriented semantic spans above Pristine's attempt telemetry.

  Events are `[:system_one_sdk, :evaluate, :start | :stop | :exception]`,
  `[:system_one_sdk, :answer]`, and `[:system_one_sdk, :batch, :cancelled]`. Durations
    ...
    |> Enum.each(fn {key, index} ->
      case Map.fetch(response.answers, key) do
        {:ok, answer} ->
          ...
          :telemetry.execute([:system_one_sdk, :answer], measurements, %{
            operation: :evaluate,
  ...
  def cancelled(index, reason) when reason in [:timeout, :task_exit] do
    :telemetry.execute([:system_one_sdk, :batch, :cancelled], %{count: 1}, %{
      operation: :evaluate,
      batch_index: index,
      outcome: reason
    })
  ...
  defp emit(event, measurements, metadata),
    do: :telemetry.execute([:system_one_sdk, :evaluate, event], measurements, metadata)
