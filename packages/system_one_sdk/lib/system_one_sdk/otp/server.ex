defmodule SystemOneSDK.OTP.Server do
  ...
  normal `SystemOneSDK.Client`. `max_in_flight` bounds pending evaluations for the
  server. SystemOneSDK still delegates HTTP, retries and physical cancellation to
  Pristine.
  ...
  alias SystemOneSDK.{Client, Error, Evaluation}
  ...
  def start_link(module, opts) when is_atom(module) and is_list(opts) do
    ...
    if unknown != [] do
      raise ArgumentError, "unknown SystemOneSDK.OTP.Server options: #{inspect(Enum.uniq(unknown))}"
    end
  ...
  @impl GenServer
  def handle_info(
        {:system_one_sdk_otp_server, marker, tag, result},
  ...
  defp route(_other, _state),
    do: raise(ArgumentError, "invalid SystemOneSDK.OTP.Server callback return")
  ...
  defp deliver_later(state, tag, result) do
    send(self(), {:system_one_sdk_otp_server, state.message_marker, tag, result})
    {:noreply, state}
  end
  ...
  defp client(opts) do
    case Keyword.get(opts, :client) do
      ...
      _ -> {:error, Error.configuration("SystemOneSDK.OTP.Server requires :client")}
    end
  end
  ...
  defp task_supervisor(opts) do
    case Keyword.get(opts, :task_supervisor) do
      nil ->
        {:error, Error.configuration("SystemOneSDK.OTP.Server requires :task_supervisor")}

      supervisor ->
        case GenServer.whereis(supervisor) do
          nil ->
            {:error, Error.configuration("SystemOneSDK.OTP.Server :task_supervisor is not running")}
