defmodule SystemOneSDK.Error do
  ...
  @spec retryable?(t(), SystemOneSDK.RetryPolicy.t() | false) :: boolean()
  def retryable?(error, policy \\ %SystemOneSDK.RetryPolicy{})
  ...
  def connection_error(reason, _opts \\ []) do
    if SystemOneSDK.TransportError.timeout?(reason) do
      timeout_error()
    else
      %__MODULE__{
        type: :connection,
