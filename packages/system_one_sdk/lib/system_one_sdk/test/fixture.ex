defmodule SystemOneSDK.Test.Fixture do
  @moduledoc false
  alias SystemOneSDK.JSON
  alias SystemOneSDK.Question.Validation
  alias SystemOneSDK.Test.ContractError

  def respond({kind, value}, request) when kind in [:answers, :models, :http_error, :response],
    do: respond({kind, value, []}, request)

  def respond({:callback, fun}, request) when is_function(fun, 1),
    do: respond(fun.(request), request)
  ...
  def respond({:http_error, status, opts}, _) do
    check!(
      is_integer(status) and status >= 400 and status <= 999,
      "HTTP error status must be >= 400"
    )
    ...
    response(
      status,
      Keyword.get(opts, :body, %{"error" => "scripted test error"}),
  ...
  def respond({:answers, specs, opts}, request) do
    ...
    check!(
      is_map(usage) and Enum.all?(Map.values(usage), &(is_integer(&1) and &1 >= 0)),
  ...
  defp answer!({:choice, choice, opts}, %{"type" => "choice", "criteria" => criteria}, id)
    ...
    probabilities =
      case Keyword.fetch(opts, :probabilities) do
        {:ok, exact} ->
          exact
          |> keyed!(["stubs", id, "probabilities"])
        ...
        :error ->
          ...
          Map.new(criteria, fn {key, _} ->
            {key, if(key == choice, do: confidence, else: (1 - confidence) / (count - 1))}
          end)
      end
  ...
  defp answer!({:score, score, opts}, %{"type" => "score", "criteria" => criteria}, id)
    ...
    check!(
      top >= 1 and is_number(score) and score >= 0 and score <= top,
    ...
    probabilities =
      case Keyword.fetch(opts, :probabilities) do
        {:ok, exact} ->
          score_probabilities!(exact, id)

        :error ->
          synthesize_score(indices, score)
      end
    ...
    legend =
      criteria
      |> Enum.with_index()
      |> Map.new(fn {label, index} -> {Integer.to_string(index), label} end)
    ...
    %{
      ...
      "confidence" => confidence,
      "legend" => legend,
      "probabilities" => Map.new(probabilities, fn {k, v} -> {Integer.to_string(k), v} end)
    }
  end

  defp answer!(_, question, id),
    do: fail!("#{id}: stub type does not match actual question #{inspect(question["type"])}")

  defp synthesize_score(indices, score) do
    low = floor(score)
    high = ceil(score)
  ...
  defp score_key!(key, _) when is_integer(key), do: key

  defp score_key!(key, id) when is_binary(key) do
  ...
  defp score_key!(_, id), do: fail!("#{id}: score probability keys must be integers")

  defp fixture_options!(opts) do
    case Validation.options(opts, [:probabilities, :confidence]) do
      :ok -> :ok
  ...
  defp distribution!(values, keys, id) do
    check!(
      MapSet.new(Map.keys(values)) == MapSet.new(keys),
      "#{id}: probability keys differ from the actual question"
    )
  ...
  defp probability!(value, field),
    do:
      check!(
        is_number(value) and value >= 0 and value <= 1,
        "#{field}: expected probability in [0, 1]"
      )

  defp endpoint!(request, endpoint),
    do:
      check!(
        String.ends_with?(URI.parse(request.url).path || "", "/v1/#{endpoint}"),
        "fixture endpoint mismatch"
      )
  ...
  defp headers(opts),
    do:
      Map.new(Keyword.get(opts, :headers, %{}), fn {k, v} ->
