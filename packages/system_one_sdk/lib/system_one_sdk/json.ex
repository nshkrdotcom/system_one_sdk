defmodule SystemOneSDK.JSON do
  @moduledoc """
  ...
  def keyed_pairs(value, path, opts \\ []) do
    ...
    with {:ok, pairs} <- pairs(value, path),
      ...
      if is_map(value),
        do: {:ok, Enum.sort_by(checked, &wire_key(elem(&1, 0)))},
        else: {:ok, checked}
    end
  end
  ...
  defp normalize(value, path, _depth) when is_float(value) do
    if abs(value) <= 1.797_693_134_862_315_7e308,
      do: {:ok, value},
      else: invalid(path, "must be a finite JSON number")
  end
  ...
  defp normalize(value, path, depth) when is_map(value) and not is_struct(value) do
    with {:ok, pairs} <- keyed_pairs(value, path, nonempty: false) do
      normalize_object(pairs, path, depth)
    end
  end

  defp normalize(_value, path, _depth), do: invalid(path, "is not a JSON value")

  defp normalize_object(pairs, path, depth) do
  ...
  defp pairs(value, _path) when is_map(value) and not is_struct(value),
    ...
    Enum.reduce_while(pairs, {:ok, [], MapSet.new()}, fn
      {key, value}, {:ok, acc, seen} when is_binary(key) or (is_atom(key) and not is_nil(key)) ->
        ...
        cond do
          ...
          true ->
            {:cont, {:ok, [{key, value} | acc], MapSet.put(seen, wire)}}
        end

      _, _ ->
        {:halt, invalid(path, "expected atom/string key and value pairs")}
    end)
    |> case do
  ...
  defp valid_key?(wire, nonempty),
    do: String.valid?(wire) and (not nonempty or String.trim(wire) != "")
