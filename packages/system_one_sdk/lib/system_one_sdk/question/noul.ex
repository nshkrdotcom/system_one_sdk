defmodule SystemOneSDK.Question.Noul do
  @moduledoc """
  Strict semantic yes/no question. `new/2` returns a tuple; `new!/2` raises.

      iex> {:ok, q} = SystemOneSDK.Question.Noul.new("Urgent?", true: "Time-sensitive")
      iex> q.criteria
      [{true, "Time-sensitive"}]
  ...
  @spec new(term(), keyword()) :: {:ok, t()} | {:error, SystemOneSDK.Error.t()}
  ...
  defp criteria_options(opts) do
    if Keyword.has_key?(opts, :criteria) and
       ...
       do:
         {:error,
          SystemOneSDK.Error.invalid_request(
            ["criteria"],
