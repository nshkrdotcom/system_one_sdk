defmodule SystemOneSDK.Models do
  ...
  def list(%Client{} = client, opts \\ []) when is_list(opts) do
    Client.list_models(client, opts)
  end

  @spec find(ListModelsResponse.t() | [ModelMetadata.t()], String.t()) ::
          {:ok, ModelMetadata.t()} | {:error, Error.t()}
  def find(catalog, name) when is_binary(name) do
    matches =
      catalog
      |> catalog()
    ...
    case matches do
      ...
      [] ->
        {:error, Error.model_lookup(:model_not_found, name)}
      ...
      many ->
        {:error,
         Error.model_lookup(
           :ambiguous_model,
           name,
           %{match_count: length(many)}
         )}
    end
  end

  def find(_catalog, name) do
    {:error,
     Error.model_lookup(
       :model_not_found,
       name,
  ...
  @spec latest(ListModelsResponse.t() | [ModelMetadata.t()], term()) ::
          {:ok, ModelMetadata.t()} | {:error, Error.t()}
  def latest(catalog, selector \\ :all) do
    catalog
    |> catalog()
    |> select(selector)
    |> objective_dates()
    |> case do
      {:ok, []} -> {:error, Error.model_lookup(:model_not_found, "<selector>")}
      {:ok, dated} -> unique_latest(dated)
      error -> error
    end
  end
  ...
  defp catalog(models) when is_list(models),
    do: Enum.filter(models, &match?(%ModelMetadata{}, &1))
  ...
  defp select(models, fun) when is_function(fun, 1),
    do: Enum.filter(models, fun)

  defp select(models, selector) when is_list(selector) do
    if Keyword.keyword?(selector),
      do: select(models, Map.new(selector)),
      else: []
  end

  defp select(models, selector)
       when is_map(selector) and not is_struct(selector) do
  ...
  defp objective_dates([]),
    do: {:error, Error.model_lookup(:model_not_found, "<selector>")}
  ...
  defp parse_release_date(value) when is_binary(value),
  ...
  defp unique_latest(dated) do
    ...
    latest =
      Enum.filter(dated, fn {date, _model} ->
        Date.compare(date, latest_date) == :eq
      end)
    ...
    case latest do
      ...
      _ ->
        {:error,
         Error.unordered_model_catalog(%{
           reason: :release_date_tie,
           count: length(latest)
         })}
    end
  end
