...
Add `{:system_one_sdk, "~> 0.6"}` to your application's dependencies once this
release is published. For a local checkout before publication, use
`{:system_one_sdk, path: "../system_one_sdk/packages/system_one_sdk"}`. Pristine `~> 0.4.0` owns HTTP
...
SystemOneSDK do not read OS environment values themselves.
...
questions = [
  ...
  severity: SystemOneSDK.score("Severity?", ["Routine", "Important", "Blocking"])
]

{:ok, response} =
  SystemOneSDK.evaluate(client, "The API is down for every customer", questions)
