defmodule SystemOneSDK.Examples.Evaluation do
  @moduledoc """
  ...
  def questions do
    SystemOneSDK.prepare!(
      department:
        SystemOneSDK.choice("Which team owns the primary support request?",
      ...
      urgent:
        SystemOneSDK.noul("Does this require urgent human attention?",
  ...
  def load!(path, split) when split in ["development", "held-out"] do
    rows =
      ...
      |> Enum.map(fn {line, number} ->
        ...
        valid =
          is_binary(row["id"]) and String.trim(row["id"]) != "" and
        ...
        case SystemOneSDK.JSON.normalize(row["state"], ["state"]) do
          {:ok, _} -> row
  ...
  def run(client, rows, opts \\ []) do
    ...
    results =
      SystemOneSDK.evaluate_many(client, Enum.map(rows, & &1["state"]), prepared,
        max_concurrency: Keyword.get(opts, :max_concurrency, 4),
        ordered: true,
        on_error: :collect,
        task_timeout_ms: Keyword.get(opts, :task_timeout_ms, 40_000)
      )
  ...
  def record(row, {:ok, response}) do
    with {:ok, %SystemOneSDK.ChoiceAnswer{} = department} <-
           SystemOneSDK.Response.fetch(response, :department),
         {:ok, %SystemOneSDK.NoulAnswer{} = urgent} <-
           SystemOneSDK.Response.fetch(response, :urgent) do
      Map.merge(labels(row), %{
        "outcome" => "ok",
        "department" => Atom.to_string(department.choice),
        "confidence" => department.confidence,
        "urgent" => urgent.noul,
        "probabilities" =>
          Map.new(department.probabilities, fn {key, p} -> {Atom.to_string(key), p} end),
  ...
  def record(row, {:error, error}) do
    Map.merge(labels(row), %{
      "outcome" => "error",
      "error_type" => Atom.to_string(error.type),
      "status" => error.status,
  ...
  def validate_policy!(_),
    do: raise(ArgumentError, "policy needs confidence and urgency thresholds")
  ...
  def metrics(records, policy) do
    ...
    required_human =
      Enum.filter(routed, &Enum.all?(&1["routes"], fn r -> r in ["review", "urgent"] end))
    ...
    %{
      ...
      "successes" => length(ok),
      "failures" => length(records) - length(ok),
      ...
      "automatic_count" => length(automatic),
      "automatic_errors" => auto_errors,
      ...
      "human_rate" => ratio(length(human), length(records)),
      ...
      "input_tokens" => token_sum(ok, "input_tokens"),
      "output_tokens" => token_sum(ok, "output_tokens"),
  ...
  def select_policy!(sweep, max_auto_error) do
    unless probability?(max_auto_error),
      do: raise(ArgumentError, "invalid maximum automatic error rate")

    eligible =
      Enum.filter(sweep, fn row ->
        m = row["metrics"]

        m["successes"] > 0 and m["automatic_count"] > 0 and
          m["automatic_error_rate"] <= max_auto_error
      end)
  ...
  defp labels?(labels, allowed),
    do: is_list(labels) and labels != [] and Enum.all?(labels, &(&1 in allowed))
