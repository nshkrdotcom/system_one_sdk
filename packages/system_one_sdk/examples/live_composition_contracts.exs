...
alias SystemOneSDK.{Error, Models, Prepared, Response}

client = Live.client()
Live.show_configuration(client)

catalog = client |> SystemOneSDK.list_models() |> Live.unwrap!()
...
base =
  SystemOneSDK.prepare!(
    urgent: SystemOneSDK.noul("Does this need prompt human attention?"),
    team:
      SystemOneSDK.choice("Which team owns this request?", [
...
{:ok, with_score} =
  Prepared.put(
    base,
    :severity,
    SystemOneSDK.score("How much business impact is described?", ["Low", "Medium", "High"])
  )
...
{:ok, replaced} =
  Prepared.put(
    with_score,
    :team,
    SystemOneSDK.choice("Which team should review this next?", [
      {:billing, "Invoices, charges, and refunds"},
      {:technical, "Product failures and integrations"},
      {:sales, "Purchasing and plan questions"}
    ])
...
right =
  SystemOneSDK.prepare!(
    team:
      SystemOneSDK.choice("Which team is the final owner?", [
        {:billing, "Billing"},
        {:technical, "Technical"},
        {:sales, "Sales"}
      ]),
    review: SystemOneSDK.noul("Should a human review the result before action?")
  )
...
probe =
  SystemOneSDK.evaluate!(probe_client, state, merged,
    timeout_ms: 12_000,
    retry: false
  )
...
response =
  SystemOneSDK.evaluate!(contract_client, state, merged,
    timeout_ms: 12_000,
    retry: false
  )
...
response_override =
  SystemOneSDK.evaluate!(contract_client, state, deleted,
...
response_unlimited =
  SystemOneSDK.evaluate!(contract_client, state, taken,
...
case SystemOneSDK.evaluate(contract_client, state, merged, max_request_bytes: 1, retry: false) do
