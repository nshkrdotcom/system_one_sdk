...
alias SystemOneSDK.{Error, Response, RuntimeCapabilities}
...
models = SystemOneSDK.list_models(client, timeout_ms: 9_000, retry: false) |> Live.unwrap!()
...
prepared =
  SystemOneSDK.prepare!(
    billing: SystemOneSDK.noul("Is this primarily about billing?"),
    route:
      SystemOneSDK.choice("Which route fits best?", [
...
case SystemOneSDK.evaluate(client, "state", prepared,
       retry: false,
...
legacy_questions = %{
  "billing" => %SystemOneSDK.Noul{instructions: "Is the supplied state about billing?"}
}
    ...
    task =
      Task.async(fn ->
        SystemOneSDK.evaluate(
          ...
          [continue: SystemOneSDK.noul("Should processing continue?")],
          cancellation: token,
          timeout_ms: 12_000,
          retry: false
        )
    ...
    batch_task =
      Task.async(fn ->
        SystemOneSDK.evaluate_many(
          ...
          [urgent: SystemOneSDK.noul("Does this need prompt attention?")],
...
if System.get_env("TYPESAFE_EXAMPLE_RETRY") == "1" do
  ...
  retry_result = SystemOneSDK.list_models(retry_client) |> Live.unwrap!()
...
caller. SystemOneSDK.OTP.Server is different: it creates a private token per worker and only mirrors
