...
alias SystemOneSDK.{Answer, Question, Response}
...
severity =
  SystemOneSDK.score("Business impact?", [
...
{:ok, prepared} = SystemOneSDK.prepare([{:urgent, urgent}, {"team", team}, {:severity, severity}])
...
for state <- states do
  response =
    client
    |> SystemOneSDK.evaluate(state, prepared,
