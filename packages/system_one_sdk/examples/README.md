# Live examples for SystemOneSDK 0.6.0

Every standalone `live_*.exs` entrypoint uses the real SystemOneSDK -> generated
...
Run from `packages/system_one_sdk` after `mix deps.get`
in the main README:
...
| `mix run examples/live_otp_server.exs` | Real `SystemOneSDK.OTP.Server`; caller-owned `Task.Supervisor`; `max_in_flight`; simultaneous callers/correlation; server/per-request options; private-vs-caller cancellation ownership; recursive `handle_evaluation/3`; shutdown cleanup | 4 |
...
| Bounded ordered/unordered batch evaluation and early-halt cleanup | `live_batching.exs` | `test/system_one_sdk/batch_test.exs`, `guides/batching.md` |
| Shared batch cancellation / no new scheduling after observed cancellation | `live_runtime_controls.exs` | `test/system_one_sdk/runtime_controls_v030_test.exs` |
...
| Protected headers and strict semantic protected `extra_body`; legacy last-write-wins extras | `live_runtime_controls.exs` | `test/system_one_sdk/runtime_test.exs` asserts exact serialized request/headers |
| Client/per-call strict response contracts + exact allowed model | `live_composition_contracts.exs` using an observed model ID | `test/system_one_sdk/response_contract_v030_test.exs`; future unknown-answer branch is test-only |
...
| Exact `Models.find/find!` and objective `Models.latest` | `live_composition_contracts.exs` using returned model IDs | `test/system_one_sdk/models_v030_test.exs`; ambiguous/invalid catalog failures are deterministic tests |
| Stable `Error.metadata` | cancellation/local validation paths in live scripts where naturally reachable | `test/system_one_sdk/metadata_v030_test.exs` |
| Semantic span telemetry | `live_observability.exs` | `test/system_one_sdk/semantic_telemetry_test.exs` |
...
| `SystemOneSDK.OTP.Server`, async replies, opaque correlation, option precedence, ownership/cleanup | `live_otp_server.exs` | `test/system_one_sdk/otp_server_v040_test.exs` covers saturation, crashes, unsupported cancellation and exact lifecycle races |
...
| Labeled development/held-out evaluation | `examples/evaluation/run.exs` | `test/system_one_sdk/evaluation_example_test.exs`, `guides/evaluating-decisions.md` |
...
`SystemOneSDK.OTP.Server` creates a **private** cancellation token for each worker.
