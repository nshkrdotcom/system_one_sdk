<p align="center">
  <img src="assets/system_one_sdk.svg" alt="SystemOneSDK" width="200" height="200"/>
</p>

<p align="center">
  <a href="https://hex.pm/packages/system_one_sdk"><img src="https://img.shields.io/hexpm/v/system_one_sdk.svg" alt="Hex.pm"/></a>
  <a href="https://hexdocs.pm/system_one_sdk"><img src="https://img.shields.io/badge/hex-docs-blue.svg" alt="HexDocs"/></a>
  <a href="https://github.com/nshkrdotcom/system_one_sdk"><img src="https://img.shields.io/badge/GitHub-repo-black?logo=github" alt="GitHub"/></a>
  <a href="https://hex.pm/packages/system_one_sdk"><img src="https://img.shields.io/hexpm/l/system_one_sdk.svg" alt="License"/></a>
</p>
...
`system_one_sdk` is the Elixir SDK for [TypeSafe AI](https://docs.typesafe.ai/introduction) and its first System One model, **Jev**.
...
```elixir
alias SystemOneSDK.{Choice, Noul, Score}
...
That is the primitive `system_one_sdk` brings to Elixir.
...
`system_one_sdk` exposes the three question families currently provided by the live System One API.
...
This table describes the retained wire-oriented structs in `SystemOneSDK`; their `instructions` field is optional and defaults to `nil`. Enforced keys require presence, not strict semantic validation. New code can use the `SystemOneSDK.Question.*` constructors described below: Choice requires 2..255 options and Score requires 2..10 levels.
...
| --- | --- |
| `SystemOneSDK.NoulAnswer` | `noul`: numeric P(true), from 0 to 1. Also retains `id` and `raw`; there is no separate wire confidence |
| `SystemOneSDK.ChoiceAnswer` | `choice`: selected label; `confidence`: certainty from 0 to 1; `probabilities`: map of labels to probabilities. Semantic calls restore caller option keys and add `id`, `raw`, and `option_order` |
| `SystemOneSDK.ScoreAnswer` | `score`: probability-weighted expected level, possibly between integers; `confidence`: certainty from 0 to 1; `legend`: level descriptions; `probabilities`: level probabilities. Both maps use integer keys starting at 0. Also retains `id`, `raw`, `level`, `label`, `description`, `levels`, and `rubric` |
...
Configure `:system_one_sdk` in your application's `config/runtime.exs`, or pass options explicitly to `SystemOneSDK.new_client/1`.
...
questions = SystemOneSDK.prepare!(
  urgent: SystemOneSDK.noul("Does this need immediate attention?"),
  team: SystemOneSDK.choice("Which team should handle this?",
    billing: "Invoices and payments", technical: "Failures and outages"),
  severity: SystemOneSDK.score("Business impact?", [
    {"Low", "Cosmetic; a workaround exists"}, {"High", "Blocking core work"}
  ])
)

{:ok, response} = SystemOneSDK.evaluate(client, "Our production integration is down", questions)
...
SystemOneSDK.Answer.gate(team, act: 0.90, review: 0.70)
```
...
Prepared sets cache validated question JSON. For multi-item evaluation, `evaluate_stream` and `evaluate_many` provide bounded supervised concurrency with explicit timeout budgets. For testing, `SystemOneSDK.Test` allows deterministic mocking while preserving serialization and validation logic.
...
SystemOneSDK provides production controls for request budgets, response validation, and scoped cancellation. Transport execution, retries, and connection lifecycle are handled by Pristine:

```elixir
cancel = Pristine.Cancellation.new()

{:ok, response} =
  SystemOneSDK.evaluate(client, state, prepared,
    cancellation: cancel,
...
```elixir
SystemOneSDK.RuntimeCapabilities.report(client).runtime
...
For process-oriented systems, SystemOneSDK includes primary-value projections, privacy-safe per-answer telemetry, and an opt-in bounded OTP server facade:
...
`[:system_one_sdk, :answer]` telemetry events carry confidence, distribution shape, and structural correlation (`answer_type`, question ordinal, model/request ID, and Prepared fingerprint) without leaking state, question text, bodies, or credentials.

`SystemOneSDK.OTP.Server` allows GenServer callbacks to run semantic workloads asynchronously without blocking the server process. The host application provides a `Task.Supervisor`, with `max_in_flight` bounding concurrent work.
...
| Struct | Fields |
| --- | --- |
| `SystemOneSDK.SystemOneResponse` | `model`, `usage`, `answers`, `request_id`, `raw_http_response`, `raw`, `unknown_answers`, `retries`, `elapsed_ms`, `runtime_elapsed_ms`, `batch_index` |
| `SystemOneSDK.Usage` | `input_tokens`, `output_tokens`; either can be `nil` |
| `SystemOneSDK.ListModelsResponse` | `models`, `request_id`, `raw_http_response`, `raw`, `retries`, `elapsed_ms` |
| `SystemOneSDK.ModelMetadata` | `name`, `description`, `release_date` (strings) |

`SystemOneSDK.SystemOneResponse.nouls/1`, `choices/1`, and `scores/1` return maps filtered by answer type. Both response modules provide `request_id!/1` and `raw_http_response!/1`, which raise a configuration error if metadata is unavailable. The corresponding fields can be `nil`; a raw HTTP response is a `%Pristine.Response{}`. `SystemOneSDK.Response` delegates these accessors and adds `fetch/2` and `fetch!/2`, which use exact caller keys. Semantic `elapsed_ms` includes local validation and decoding; `runtime_elapsed_ms` retains Pristine timing. Batch indexes are zero-based, and errors store their index in `details.batch_index`.
...
```elixir
{:ok, available} = SystemOneSDK.list_models(client)
...
```elixir
{:ok, result} = SystemOneSDK.system_one(client, state, questions)
...
Host applications configure `:system_one_sdk` in application config or pass options explicitly when creating a client.

| Client option | Application key under `:system_one_sdk` | Default / units |
| --- | --- | --- |
...
| `:retry` | `:retry` | Default `SystemOneSDK.RetryPolicy`; `false` disables retries |
...
`SystemOneSDK.system_one/4` accepts `:model`, `:timeout`, `:timeout_ms`, `:retry`, `:extra_headers`, and `:extra_body`. `SystemOneSDK.list_models/2` accepts the timeout, retry, and extra-header options. Per-call non-nil timeout/retry values override the client; `:timeout_ms` wins over `:timeout`. A truthy per-call `:model` replaces the client default. A retry map or keyword list merges into the client policy: omitted fields inherit, explicit fields win, and `false` disables retries for that call.
...
```elixir
case SystemOneSDK.system_one(client, state, questions) do
  {:ok, response} ->
    response

  {:error, %SystemOneSDK.Error{} = error} ->
    %{
      type: error.type,
      status: error.status,
...
`%SystemOneSDK.Error{}` fields are `type`, `message`, `status`, `body`, `headers`, `request_id`, `retry_after_ms`, `field_path`, `path`, `endpoint`, `raw_http_response`, and `details`.
...
- `SystemOneSDK.new_client/1` raises a configuration error if no API key is available; invalid timeout/retry configuration can also raise.
- Legacy `system_one` question normalization failures return `{:error, %SystemOneSDK.Error{type: :configuration}}`.
...
`SystemOneSDK.Error.retryable?(error, client.retry)` checks policy eligibility;
...
```elixir
validation_client = SystemOneSDK.new_client(api_key: "local-validation-only")
{:error, %SystemOneSDK.Error{type: :configuration, message: message}} =
  SystemOneSDK.system_one(validation_client, "state", %{})
IO.puts(message)
```
...
`mix run examples/live_runtime_controls.exs` exercises both behaviors through the real API while `test/system_one_sdk/runtime_test.exs` asserts the exact serialized body/header contract.
...
The catalog maps all public 0.2-0.4 capabilities to runnable coverage and labels honest test-only exceptions such as `SystemOneSDK.Test`, future unknown answer types, deterministic overload/race failures, codegen freshness, and architecture gates. The evaluation CLI and live recorder also accept explicit `--base-url` / `--model` overrides that win over environment defaults.
...
`system_one_sdk` deliberately stays small.
...
`system_one_sdk` provides Elixir ergonomics and semantic validation, relying on Pristine for robust HTTP transport, retries, and execution policies.
...
with {:ok, evaluation} <- SystemOneSDK.system_one(client, state, routing_questions),
...
System One allows applications to express these questions as typed function calls with bounded outputs and calibrated probabilities. `system_one_sdk` makes that primitive native to Elixir.

---

# Documentation

The [0.4.0 cheatsheet](cheatsheets/system_one_sdk.cheatmd) covers strict constructors,
...
SystemOneSDK is an independent Elixir SDK, designed with inspiration from early community implementations:
