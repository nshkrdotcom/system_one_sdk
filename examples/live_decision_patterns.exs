...
response =
  SystemOneSDK.evaluate!(
    ...
    relevance: SystemOneSDK.score("Relevance to the query?", ["Unrelated", "Partial", "Direct"]),
    clarity: SystemOneSDK.score("Clarity of the instructions?", ["Unclear", "Mixed", "Clear"])
  )
...
lookup = Task.Supervisor.async_nolink(supervisor, fn -> SystemOneSDK.list_models(client) end)

try do
  decision =
    SystemOneSDK.evaluate!(client, "Which TypeSafe models are available?",
      useful:
        SystemOneSDK.noul("Would a list of available TypeSafe models help answer this request?")
    )
