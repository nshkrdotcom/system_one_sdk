...
alias SystemOneSDK.{Response, RuntimeCapabilities}

# Exported handler avoids telemetry's anonymous-handler performance warning.
defmodule SystemOneSDK.Examples.LiveTelemetry do
...
id = {SystemOneSDK.Examples.LiveTelemetry, make_ref()}

events =
  Enum.map([:start, :stop, :exception], &[:system_one_sdk, :evaluate, &1]) ++
    [[:system_one_sdk, :answer]]

:ok = :telemetry.attach_many(id, events, &SystemOneSDK.Examples.LiveTelemetry.handle/4, self())
...
prepared =
  SystemOneSDK.prepare!(
    billing: SystemOneSDK.noul("Is this primarily a billing request?"),
    team:
      SystemOneSDK.choice("Which team owns this?", [
        {:billing, "Billing"},
        {:technical, "Technical"},
        {:sales, "Sales"}
      ]),
    severity: SystemOneSDK.score("How severe is the impact?", ["Low", "Medium", "High"])
  )
...
try do
  ...
  expected = [
    {[:system_one_sdk, :evaluate, :start], nil, nil},
    {[:system_one_sdk, :answer], :noul, 0},
    {[:system_one_sdk, :answer], :choice, 1},
    {[:system_one_sdk, :answer], :score, 2},
    {[:system_one_sdk, :evaluate, :stop], nil, nil}
  ]
