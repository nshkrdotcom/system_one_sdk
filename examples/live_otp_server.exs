...
defmodule SystemOneSDK.Examples.LiveOTPServer do
  use SystemOneSDK.OTP.Server

  alias SystemOneSDK.{Error, Response}
  ...
  def handle_call({:classify, correlation, state, opts}, from, inner) do
    questions = [
      route:
        SystemOneSDK.choice("Which support route best matches this request?", [
  ...
  def handle_call({:recursive, correlation, state}, from, inner) do
    questions = [
      area:
        SystemOneSDK.choice("Which broad area best matches this request?", [
          {:billing, "Charges, invoices, and refunds"},
          {:technical, "Product failures and integrations"}
        ])
    ]
  ...
  def handle_evaluation(
    ...
    if depth < max_depth and remaining > 1 do
      ...
      questions = [detail: SystemOneSDK.choice("Which narrower workflow best matches?", options)]
...
{:ok, server} =
  SystemOneSDK.Examples.LiveOTPServer.start_link(
    client: client,
