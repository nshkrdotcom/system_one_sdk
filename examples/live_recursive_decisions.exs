...
alias SystemOneSDK.Examples.Live

defmodule SystemOneSDK.Examples.RecursivePatterns do
  alias SystemOneSDK.{Answer, Response}
  ...
  def coarse_to_fine(client, state) do
    ...
    coarse_response =
      evaluate!(client, state,
        route: SystemOneSDK.choice("Which broad route best matches?", options)
      )
    ...
    if margin < 0.25 do
      ...
      fine_response =
        evaluate!(client, state,
          route: SystemOneSDK.choice("Choose between the two strongest remaining routes.", narrowed)
        )
  ...
  def clarifying_conversation(client) do
    ...
    question =
      SystemOneSDK.choice("Which route best matches the user's issue?", [
  ...
  defp bisect(client, state, chunks, depth, max_depth, remaining) do
    ...
    response =
      evaluate!(
        ...
        left: SystemOneSDK.noul("Does the left half contain the better match for the query?"),
        right: SystemOneSDK.noul("Does the right half contain the better match for the query?")
      )
  ...
  defp verify_loop(client, state, candidates, round, max_rounds, requests) do
    ...
    verify_response =
      evaluate!(client, %{state: state, candidate: to_string(selection)},
        wrong: SystemOneSDK.noul("Is the selected candidate route wrong for this request?")
      )
  ...
  defp choose(client, state, id, prompt, options) do
    response = evaluate!(client, state, [{id, SystemOneSDK.choice(prompt, options)}])
    response |> Response.values() |> Map.fetch!(id)
  end

  defp evaluate!(client, state, questions) do
    SystemOneSDK.evaluate!(client, state, questions, timeout_ms: @timeout_ms, retry: false)
  end
end
...
{hierarchical, hierarchical_requests} =
  SystemOneSDK.Examples.RecursivePatterns.hierarchical_descent(client, state)

Live.show("Hierarchical descent", hierarchical)

{bisection, bisection_requests} =
  SystemOneSDK.Examples.RecursivePatterns.bisection(
    client,
...
{verify_repair, verify_requests} =
  SystemOneSDK.Examples.RecursivePatterns.verify_repair(
    client,
...
{coarse_fine, coarse_requests} =
  SystemOneSDK.Examples.RecursivePatterns.coarse_to_fine(
    client,
...
{clarifying, clarifying_requests} =
  SystemOneSDK.Examples.RecursivePatterns.clarifying_conversation(client)
