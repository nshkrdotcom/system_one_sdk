...
SystemOneSDK 0.4.0 is additive. It keeps the generated OpenAPI operations,
...
`SystemOneSDK.Response.values/1` returns only the primary semantic values while
...
Successful semantic evaluations now emit one `[:system_one_sdk, :answer]` event per
...
`SystemOneSDK.OTP.Server` is optional and starts no global SDK process. Applications
provide a running `Task.Supervisor`, a `SystemOneSDK.Client`, and an explicit
