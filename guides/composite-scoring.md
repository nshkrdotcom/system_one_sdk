...
```elixir
questions = [
  relevance: SystemOneSDK.score("How relevant is this passage?", ["Unrelated", "Partial", "Direct"]),
  clarity: SystemOneSDK.score("How clear is this passage?", ["Unclear", "Mixed", "Clear"])
]
{:ok, response} = SystemOneSDK.evaluate(client, passage, questions)
weights = %{relevance: 0.7, clarity: 0.3}
combined = Enum.reduce(weights, 0.0, fn {id, weight}, total ->
  answer = SystemOneSDK.Response.fetch!(response, id)
  total + weight * SystemOneSDK.Answer.Score.normalized(answer)
end)
```
