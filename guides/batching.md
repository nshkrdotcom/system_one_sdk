...
```elixir
prepared = SystemOneSDK.prepare!(urgent: SystemOneSDK.noul("Urgent?"))
results = SystemOneSDK.evaluate_many(client, states, prepared,
...
```elixir
SystemOneSDK.evaluate_stream(client, large_enumerable, prepared, ordered: false)
|> Enum.each(fn
...
stream = SystemOneSDK.evaluate_stream(client, states, prepared,
  cancellation: cancel,
  max_concurrency: 8)
