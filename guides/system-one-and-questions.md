...
`SystemOneSDK.evaluate/4`. This page documents the retained `system_one/4` parity
interface, whose constructors still return legacy structs.

`SystemOneSDK.system_one/4` evaluates text or structured JSON-compatible state and
...
```elixir
{:ok, response} = SystemOneSDK.system_one(client, state, questions)
