...
question. SystemOneSDK 0.4.0 documents these patterns explicitly; the SDK does not
introduce a separate workflow engine. Use ordinary functions, `SystemOneSDK.evaluate/4`,
`Prepared`, cancellation, supervisors, or `SystemOneSDK.OTP.Server` as appropriate.
...
SystemOneSDK's bounded/runtime separation rather than copying Jev's transport.
...
def descend(client, state, node, depth, max_depth) when depth < max_depth do
  ...
  if length(children) < 2 do
    ...
    with {:ok, response} <-
           SystemOneSDK.evaluate(client, state,
             child: SystemOneSDK.choice("Which child is the best match?", options)
           ),
         answer <- SystemOneSDK.Response.fetch!(response, :child),
         :act <- SystemOneSDK.Answer.gate(answer, act: 0.85, review: 0.65),
...
`SystemOneSDK.OTP.Server` can express the same loop by returning another
...
workflow/supervision system and keep SystemOneSDK as the decision primitive.
