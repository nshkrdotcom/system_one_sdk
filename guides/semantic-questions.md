...
```elixir
{:ok, q} = SystemOneSDK.Question.Noul.new("Urgent?", true: "Time-sensitive")
q = SystemOneSDK.Question.Noul.new!("Urgent?", false: "Routine")
q = SystemOneSDK.noul("Urgent?", criteria: %{"true" => "Time-sensitive"})

{:ok, team} = SystemOneSDK.Question.Choice.new("Which team?",
  billing: "Money", technical: "Product")
severity = SystemOneSDK.score("Impact?", [
...
```elixir
q = SystemOneSDK.noul("Is this relevant?", extra: %{future_flag: true})
{:ok, prepared} = SystemOneSDK.prepare(%{
...
```elixir
prepared = SystemOneSDK.prepare!(team: team, severity: severity)
SystemOneSDK.evaluate(client, "first state", prepared)
SystemOneSDK.evaluate(client, "second state", prepared)
```
