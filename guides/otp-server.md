...
SystemOneSDK 0.4.0 adds `SystemOneSDK.OTP.Server`, an opt-in GenServer facade for
...
but keeps SystemOneSDK's existing response structs, Pristine runtime, cancellation
...
defmodule MyApp.Triage do
  ...
  def handle_call({:route, ticket}, from, state) do
    questions = [
      team: SystemOneSDK.choice("Which team?", billing: nil, support: nil)
    ]
  ...
  def handle_evaluation({:ok, response}, from, state) do
    GenServer.reply(from, {:ok, SystemOneSDK.Response.values(response)})
...
option-validation path as direct `SystemOneSDK.evaluate/4` calls.
...
`%SystemOneSDK.Error{type: :runtime_capability}` instead of creating another task.
...
saturation/failure assertions remain in `test/system_one_sdk/otp_server_v040_test.exs`;
