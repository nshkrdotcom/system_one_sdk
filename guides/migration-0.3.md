...
request =
  Task.async(fn ->
    SystemOneSDK.evaluate(client, state, prepared, cancellation: cancel)
  end)
...
```elixir
report = SystemOneSDK.RuntimeCapabilities.report(client)
...
SystemOneSDK.evaluate(client, state, prepared, retry: [max_retries: 1])

# Explicitly disable for this call.
SystemOneSDK.evaluate(client, state, prepared, retry: false)
```
...
prepared = SystemOneSDK.prepare!(
  ...
  team: SystemOneSDK.choice("Team?", billing: "Billing", product: "Product")
)
...
{:ok, updated} =
  SystemOneSDK.Prepared.put(prepared, :severity, SystemOneSDK.score("Severity?", ["Low", "High"]))

SystemOneSDK.Prepared.fingerprint(updated)
...
```elixir
client = SystemOneSDK.new_client(api_key: key, max_request_bytes: 262_144)

# A per-call value wins. Explicit nil disables the client budget for this call.
SystemOneSDK.evaluate(client, state, prepared, max_request_bytes: 131_072)
```
...
```elixir
{:ok, catalog} = SystemOneSDK.list_models(client)
{:ok, model} = SystemOneSDK.Models.find(catalog, "jev")
model = SystemOneSDK.Models.find!(catalog.models, "jev")
{:ok, latest} = SystemOneSDK.Models.latest(catalog, :all)
```
...
SystemOneSDK.Error.metadata(error)
```
