...
defmodule SystemOneSDK.LiveExampleConfigurationTest do
  ...
  setup do
    ...
    previous_app = Application.get_all_env(:system_one_sdk)
    ...
    on_exit(fn ->
      ...
      for {key, _value} <- Application.get_all_env(:system_one_sdk),
          do: Application.delete_env(:system_one_sdk, key)

      for {key, value} <- previous_app, do: Application.put_env(:system_one_sdk, key, value)
    end)
  ...
  test "application config is used when no live environment override is selected" do
    Application.put_env(:system_one_sdk, :api_key, "configured-key")
    Application.put_env(:system_one_sdk, :base_url, "https://configured.example/root")
  ...
  test "caller cannot swap a fixture transport into a live example client" do
    ...
    client =
      Live.client(
        transport: SystemOneSDK.Test.Transport,
