defmodule SystemOneSDK.BatchTest do
  use ExUnit.Case, async: true
  alias SystemOneSDK.{Error, Test}
  defp questions, do: [q: SystemOneSDK.noul("Q?")]

  test "preparation and shared-option failures occur before spawning requests" do
    client = Test.client()
    assert_raise Error, fn -> SystemOneSDK.evaluate_many(client, ["x"], [], on_error: :collect) end
    ...
    for opts <- [
      ...
      assert_raise Error, fn -> SystemOneSDK.evaluate_many(client, ["x"], questions(), opts) end
    end
  ...
  test "stream is cold, reusable, and records one result per state including validation errors" do
    ...
    stream = SystemOneSDK.evaluate_stream(client, ["one", self(), "three"], questions())
    assert Test.requests(client) == []
  ...
  test "concurrency is bounded and early halt kills remaining owned tasks" do
    ...
    consumer =
      Task.async(fn ->
        client
        |> SystemOneSDK.evaluate_stream(Stream.iterate(0, &(&1 + 1)), questions(),
          max_concurrency: 2
        )
        |> Enum.take(1)
      end)
  ...
  test "per-state timeout preserves index and never reports the raw state as an exit reason" do
    ...
    assert [{:error, error}] =
             SystemOneSDK.evaluate_many(client, ["private-state"], questions(), task_timeout_ms: 30)
  ...
  test "unordered results retain association with input indexes" do
    ...
    consumer =
      Task.async(fn ->
        SystemOneSDK.evaluate_stream(client, [0, 1], questions(),
          ordered: false,
          max_concurrency: 2
        )
        |> Enum.map(fn result ->
  ...
  test "caller death cleans the supervisor and its tasks" do
    ...
    consumer =
      spawn(fn -> SystemOneSDK.evaluate_many(client, [0, 1], questions(), max_concurrency: 2) end)
  ...
  test "on_error raise uses the normalized SDK error, and attempts use millisecond option" do
    ...
    assert_raise Error, fn ->
      SystemOneSDK.evaluate_many(client, ["x"], questions(), on_error: :raise)
    end

    client = Test.stub(client, q: {:noul, 0.8})

    assert [{:ok, _}] =
             SystemOneSDK.evaluate_many(client, ["x"], questions(), attempt_timeout_ms: 777)
  ...
  test "stream cleanup does not leak lifecycle EXIT messages into a trapping caller" do
    ...
    try do
      ...
      assert [{:ok, _}] = SystemOneSDK.evaluate_many(client, ["x"], questions())
  ...
  test "an exited worker becomes a per-input error instead of killing the caller" do
    ...
    assert [{:error, %Error{} = error}] =
             SystemOneSDK.evaluate_many(client, ["x"], questions(), max_concurrency: 1)
  ...
  test "ordered prefetch and completed-result buffering cannot run past the declared window" do
    ...
    consumer =
      Task.async(fn ->
        SystemOneSDK.evaluate_stream(client, states, questions(),
          max_concurrency: 2,
          max_pending: 3,
          ordered: true
        )
    ...
    assert_raise Error, fn ->
      SystemOneSDK.evaluate_stream(client, [1], questions(), max_concurrency: 2, max_pending: 1)
    end
  end
end
