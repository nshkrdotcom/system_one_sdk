defmodule SystemOneSDK.EvaluationTest do
  use ExUnit.Case, async: true
  alias SystemOneSDK.{Error, Test}
  ...
  test "real serialization and decoding retain IDs, distributions and metadata", %{client: client} do
    ...
    prepared =
      SystemOneSDK.prepare!(
        team: SystemOneSDK.choice("Team?", support: "Help", billing: "Money"),
        level: SystemOneSDK.score("Severity?", ["Low", "High"])
      )

    assert {:ok, response} = SystemOneSDK.evaluate(client, %{document: ["synthetic"]}, prepared)
  ...
  test "ordered Choice bytes survive Pristine with more than 32 options", %{client: client} do
    ...
    assert {:ok, response} =
             SystemOneSDK.evaluate(client, "synthetic", q: SystemOneSDK.choice("Pick", pairs))
  ...
  test "legacy system_one still has string keys and legacy constructor return values", %{
    ...
    question = SystemOneSDK.Noul.new(instructions: "Question?")
    assert %SystemOneSDK.Noul{} = question
    response = SystemOneSDK.system_one!(client, "synthetic", %{q: question})
  ...
  test "invalid requests and semantic override attempts never reach transport", %{client: client} do
    question = [q: SystemOneSDK.noul("Question?")]
    assert {:error, %Error{path: ["state"]}} = SystemOneSDK.evaluate(client, self(), question)

    assert {:error, %Error{path: ["options", "extra_body", "questions"]}} =
             SystemOneSDK.evaluate(client, "x", question, extra_body: %{questions: %{}})
    ...
    for opts <- [
      ...
      assert {:error, %Error{type: :invalid_request}} =
               SystemOneSDK.evaluate(client, "x", question, opts)
    end
  ...
  test "local extras and per-call timeout reach the normal runtime", %{client: client} do
    client = Test.stub(client, q: {:noul, 0.9})
    question = SystemOneSDK.Question.Noul.new!("Q?", extra: %{experimental: true})

    assert {:ok, _} =
             SystemOneSDK.evaluate(client, "x", [q: question],
               extra_body: %{future: 4},
               timeout_ms: 1234,
               model: "jev-override"
             )
  ...
  test "response-relative errors retain actual HTTP metadata", %{client: client} do
    ...
    assert {:error, %Error{type: :response_validation, status: 200, request_id: "req-bad"} = error} =
             SystemOneSDK.evaluate(client, "x", q: SystemOneSDK.noul("Q?"))
  ...
  test "bang evaluation raises the normalized error", %{client: client} do
    ...
    assert_raise Error, fn -> SystemOneSDK.evaluate!(client, "x", q: SystemOneSDK.noul("Q?")) end
  end
  ...
  test "finite sequences exercise real provider retries and retry count headers" do
    ...
    assert {:ok, response} = SystemOneSDK.evaluate(client, "x", q: SystemOneSDK.noul("Q?"))
  ...
  test "retry advice is public and respects classification and configured flags", %{client: client} do
    ...
    assert {:error, error} = SystemOneSDK.evaluate(client, "x", q: SystemOneSDK.noul("Q?"))
    ...
    policy = SystemOneSDK.RetryPolicy.new!(api_timeout_error: false)
  ...
  test "test fixtures validate actual question contracts and verification cannot hide failures", %{
    ...
    try do
      SystemOneSDK.evaluate(client, "x", new_id: SystemOneSDK.noul("Q?"))
    rescue
  ...
  test "model stubs and bounded history retain real decoded metadata" do
    ...
    for _ <- 1..3, do: assert({:ok, _} = SystemOneSDK.list_models(client))
    assert %{total: 3, dropped: 1, requests: requests} = Test.stats(client)
    assert length(requests) == 2
    {:ok, models} = SystemOneSDK.list_models(client)
  ...
  test "pending and exhausted sequences are observable", %{client: client} do
    ...
    assert {:ok, _} = SystemOneSDK.evaluate(client, "x", q: SystemOneSDK.noul("Q?"))
  ...
  test "fixture backoff and breaker state are isolated and cleaned with the scenario" do
    ...
    assert {:error, %Error{status: 429}} =
             SystemOneSDK.evaluate(limited, "limited", q: SystemOneSDK.noul("Q?"))

    task =
      Task.async(fn -> SystemOneSDK.evaluate(healthy, "healthy", q: SystemOneSDK.noul("Q?")) end)
