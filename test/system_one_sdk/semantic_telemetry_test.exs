defmodule SystemOneSDK.SemanticTelemetryTest do
  use ExUnit.Case, async: true
  alias SystemOneSDK.{Telemetry, Test}
  ...
  setup do
    ...
    events =
      Enum.map([:start, :stop, :exception], &[:system_one_sdk, :evaluate, &1]) ++
  ...
  test "one semantic span has safe metadata, nested caller context and real token counts", %{
    ...
    assert {:ok, _} =
             SystemOneSDK.evaluate(
               client,
               "private-state",
               [q: SystemOneSDK.noul("private-question")],
               telemetry_metadata: caller
             )

    assert_receive {[:system_one_sdk, :evaluate, :start], _, start}
    assert_receive {[:system_one_sdk, :answer], answer_measurements, answer_metadata}
    assert_receive {[:system_one_sdk, :evaluate, :stop], measurements, stop}
  ...
  test "per-answer events expose distribution shape but not answer values or question IDs", %{
    ...
    questions =
      SystemOneSDK.prepare!(
        private_route: SystemOneSDK.choice("secret route", billing: nil, support: nil),
        private_score: SystemOneSDK.score("secret score", ["low", "medium", "high"])
      )

    assert {:ok, _} =
             SystemOneSDK.evaluate(client, "secret state", questions, telemetry_metadata: caller)

    assert_receive {[:system_one_sdk, :answer], choice, %{answer_type: :choice} = choice_meta}
    assert_receive {[:system_one_sdk, :answer], score, %{answer_type: :score} = score_meta}
  ...
  test "invalid local input has a terminal classified event", %{metadata: caller} do
    ...
    assert {:error, _} =
             SystemOneSDK.evaluate(client, self(), [q: SystemOneSDK.noul("Q?")],
               telemetry_metadata: caller
             )

    assert_receive {[:system_one_sdk, :evaluate, :stop], _, %{error_type: :invalid_request}}
  ...
  test "future answers emit no answer event and known events keep Prepared order", %{
    ...
    assert {:ok, response} =
             SystemOneSDK.evaluate(
               client,
               "state",
               [future: SystemOneSDK.noul("Future?"), known: SystemOneSDK.noul("Known?")],
               telemetry_metadata: caller
             )

    assert SystemOneSDK.Response.values(response) == %{known: 0.8}
    assert_receive {start_event, _, _}
    assert start_event == [:system_one_sdk, :evaluate, :start]
    assert_receive {answer_event, _, answer_metadata}
    assert answer_event == [:system_one_sdk, :answer]
    assert answer_metadata.question_index == 1
    assert_receive {stop_event, _, _}
    assert stop_event == [:system_one_sdk, :evaluate, :stop]
    refute_receive {[:system_one_sdk, :answer], _, _}
  end
  ...
  test "exception event excludes reason and stacktrace while preserving the caller exception", %{
    ...
    assert_receive {[:system_one_sdk, :evaluate, :exception], _, metadata}
